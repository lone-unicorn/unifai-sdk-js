...
import { EtherSigner, WagmiSigner, isWagmiSigner, isEtherSigner } from '../types';
import { RateLimiter } from '../../common';

export class EVMHandler {
    constructor(private rateLimiter?: RateLimiter) {}

    async sendTransaction(signer: EtherSigner | WagmiSigner, tx: any): Promise<{ hash: string | undefined }> {
        try {
