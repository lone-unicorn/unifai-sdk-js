...
export class SolanaHandler {
    constructor(
        ...
        private maxPollTimes: number = 18,
        private pollInterval: number = 5000
    ) {}

    async sendTransaction(signer: SolanaSigner, tx: any, config?: SendConfig): Promise<{ hash: string | undefined }> {
        try {
            ...
            let pollResult = this.pollTransactionStatus(finalConnection, signature, this.maxPollTimes, this.pollInterval, abortController.signal);
            let wsResult = this.waitTransactionConfirmed(finalConnection, signature, signedTransaction);
